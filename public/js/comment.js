...
$('#messageBtn').on('click', function() {
    $.ajax({
        ...
        success: function(responseData) {
            ...
            $('#messageContent').val('');
            comments = responseData.data.comments.reverse();//reverse() 方法用于颠倒数组中元素的顺序。
            renderComment();
        }
    })
});
...
$.ajax({
    ...
    success: function(responseData) {
        ...
        comments =responseData.data.reverse(); //reverse() 方法用于颠倒数组中元素的顺序。
        renderComment();
    }
});
...
function renderComment() {
    ...
    if (page <= 1) {
        page = 1;
        $lis.eq(0).html('<span style="color:#b7b7b7">没有上一页了</span>');
    } else {
        $lis.eq(0).html('<a href="javascript:;" >上一页</a>');
    }
    if (page >= pages) {
        page = pages;
        $lis.eq(2).html('<span style="color:#b7b7b7">没有下一页了</span>');
    } else {
